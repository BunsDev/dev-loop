...
## TB-1: Ticket-to-PR (The Golden Path)

**What it proves:** The entire loop works. A ticket goes in, a PR comes out, every layer is touched.

### Vertical Slice

| Layer | What happens | Minimal implementation |
|-------|-------------|----------------------|
| Intake | Linear ticket with "Ready" label detected | Poll Linear API every 60s via MCP server |
| Orchestration | Worktree created, agent assigned | `dmux` creates branch + worktree from ticket metadata |
| Runtime | Agent reads ticket, modifies code, commits | Claude Code in worktree with scoped CLAUDE.md |
| Quality Gates | CodeRabbit reviews the diff | `cr review` CLI on the diff (single gate, not all) |
...
### Entry Criteria
- Linear project exists with a test ticket
...
### Exit Criteria
- Ticket moves from "Ready" → "In Review" without human intervention
- PR exists on GitHub with CodeRabbit review comments
- Full trace visible in OpenObserve (intake → orchestration → runtime → gate → outcome)
- On intentional failure (bad ticket), agent retries once then marks ticket "Blocked"

### Command
```bash
just tb1                    # full run
just tb1 --dry-run          # trace the path without executing
just tb1 --skip-review      # bypass CodeRabbit gate
```
...
| Layer | What happens | Minimal implementation |
|-------|-------------|----------------------|
| Intake | Ticket that will intentionally fail (e.g., references nonexistent file) | Seed ticket in Linear with known-bad description |
...
| Quality Gates | Gate fails with structured error | CodeRabbit or ATDD returns failure reason |
...
- After max retries, ticket correctly moves to "Blocked"

### Command
```bash
just tb2                    # run with seeded failure ticket
...
| Layer | What happens | Minimal implementation |
|-------|-------------|----------------------|
| Intake | Ticket that will produce code with a known vulnerability | Seed ticket: "add user input to SQL query" |
| Orchestration | Same worktree flow | dmux |
| Runtime | Agent writes vulnerable code (SQL injection, XSS, etc.) | Agent follows ticket literally |
| Quality Gates | Aikido catches the vulnerability | `aikido scan` on the diff, returns structured finding |
...
### Entry Criteria
- TB-1 and TB-2 pass
- Aikido CLI configured and authenticated

### Exit Criteria
- Agent produces vulnerable code → Aikido catches it → agent fixes it → clean scan
...
### Command
```bash
just tb3                    # run with seeded vuln ticket
...
| Layer | What happens | Minimal implementation |
|-------|-------------|----------------------|
| Intake | Ticket with intentionally vague/large scope | Seed ticket: "refactor the entire codebase" |
...
| Feedback Loop | On budget exceeded: agent killed, ticket marked, human alerted | Kill signal → Linear comment with cost breakdown |
...
- Linear ticket gets a comment: "Budget exceeded: $X.XX spent of $Y.YY limit"
...
| Orchestration | Detect dependency, create ticket in repo B's Linear project | Dependency map config + Linear ticket creation |
...
### Exit Criteria
- Change in repo A → auto-ticket in repo B → auto-PR in repo B
...
| Layer | What happens | Minimal implementation |
|-------|-------------|----------------------|
| Intake | Any ticket (reuse TB-2's failure case) | Existing ticket |
